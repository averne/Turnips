...
static_assert(turnip_offsets.size()  == versions.size());
static_assert(visitor_offsets.size() == versions.size());
...
class TurnipParser {
    ...
    public:
        ...
        inline std::string get_pattern() const {
            return impl::turnip_patterns[this->prices.pattern_type];
        }

    private:
        inline std::size_t get_tp_offset() const {
            return (this->version != Version::Unknown) ? impl::turnip_offsets[static_cast<std::size_t>(this->version)] : 0ul;
        }
...
class VisitorParser {
    ...
    public:
        ...
        inline VisitorNames get_visitor_names() const {
            ...
            [](std::uint32_t visitor) {
                return impl::visitor_names[visitor];
            });
            return names;
        }
    ...
    private:
        inline std::size_t get_vs_offset() const {
            return (this->version != Version::Unknown) ? impl::visitor_offsets[static_cast<std::size_t>(this->version)] : 0ul;
        }
