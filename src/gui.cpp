...
namespace gui
{
    ...
    {
        if (!im::BeginTabItem(Lang::stringtochar("Turnips"_lang)))
            return;
        ...
        float average = static_cast<float>(std::accumulate(prices.week_prices.begin() + 2,
                                                           prices.week_prices.end(), 0)) /
                        (prices.week_prices.size() - 2);

        im::Text(Lang::stringtochar("price_pattern"_lang), prices.buy_price, pattern.c_str());

        im::BeginTable("##Prices table", 3, ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersH | ImGuiTableFlags_BordersV);
        im::TableNextRow();
        im::TableNextCell(), im::TextUnformatted(Lang::stringtochar("AM"_lang));
        im::TableNextCell(), im::TextUnformatted(Lang::stringtochar("PM"_lang));
        ...
        day_names[0] = "Sunday"_lang;
        day_names[1] = "Monday"_lang;
        day_names[2] = "Tuesday"_lang;
        day_names[3] = "Wednesday"_lang;
        day_names[4] = "Thursday"_lang;
        day_names[5] = "Friday"_lang;
        day_names[6] = "Saturday"_lang;
        auto print_day = [&](std::uint32_t day) -> void {
            im::TableNextRow();
            im::TextUnformatted(day_names[day].c_str());
        ...
        im::Separator();
        do_with_color(th::text_max_col, [&] { im::Text(Lang::stringtochar("MAX"_lang), max); });
        im::SameLine();
        do_with_color(th::text_min_col, [&] { im::Text(Lang::stringtochar("MIN"_lang), min); });
        im::SameLine();
        im::Text(Lang::stringtochar("arg"_lang), average);

        im::Separator();
        im::TextUnformatted(Lang::stringtochar("week_graph"_lang));
    ...
    {
        if (!im::BeginTabItem(Lang::stringtochar("visitors"_lang)))
            return;
        ...
        auto print_day = [&](std::uint32_t day) -> void {
            ...
            do_with_color(get_color(day), [&] {
                ...
                if (day == parser.get_celeste_day())
                    im::SameLine(), im::TextUnformatted(Lang::stringtochar("celeste"_lang));
                if (day == parser.get_wisp_day())
                    im::SameLine(), im::TextUnformatted(Lang::stringtochar("wisp"_lang));
            });
        };
    ...
    {
        if (!im::BeginTabItem(Lang::stringtochar("weather"_lang)))
            return;

        auto seed = parser.calculate_weather_seed();

        im::Dummy(ImVec2(0.0f, 10.0f));
        im::Text(Lang::stringtochar("Hemisphere"_lang), parser.get_hemisphere_name().c_str());
        im::Text(Lang::stringtochar("weather_seed"_lang), seed, seed);

        im::Separator();
        im::TextUnformatted(Lang::stringtochar("tips"_lang));

        im::EndTabItem();
    }
    void draw_language_tab(int e_setting_language)
    {
        if (!im::BeginTabItem(Lang::stringtochar("lanuage_setting"_lang)))
            return;
        im::RadioButton("English", &e_setting_language, Language::English);
        im::RadioButton("中文", &e_setting_language, Language::Chinese);
